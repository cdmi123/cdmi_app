...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$('#reportname').submit(function(e){
          ...
          var formdata = $(this).serialize();
        	$.ajax({
         		type:"POST",
          		url:"<?php echo site_url('progress_report/add_sub_topic');?>",
          		data:formdata,
          		success:function(res)
          		{
            		$('#report-topic-name').html(res);
      ...
      $('#select_report').change(function(){
          ...
          localStorage.setItem("progress_report.cat_id",report_id);
          $.ajax({

            type:"POST",
              url:"<?php echo site_url('progress_report/ajax_sub_topic');?>",
  ...
  $(document).ready(function(){
    $('#updateTopicForm').submit(function(e){
          ...
          $.ajax({
            type:"POST",
              url:"<?php echo site_url('progress_report/update_topic');?>",
              data:formdata,
              success:function(res)
              {
                $('#report-topic-name').html(res);
  ...
  $(document).on('click','.delete_sub_topic',function(){
        var del_id = $(this).attr('data-delete');
        var x = localStorage.getItem("progress_report.cat_id");

       $.ajax({
            type:"POST",
              url:"<?php echo site_url('progress_report/delete_sub_topic');?>",
