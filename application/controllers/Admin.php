...
class Admin extends CI_Controller
{
	...
	function index($id=0)
	{
		...
		if($this->input->post('submit'))
		{
			...
			$punchcode=$this->input->post('punchcode');
			
			$arr=array('name'=>$name,'email'=>$email,'mobile'=>$contact,'status'=>$status,'role'=>$role,'branch_id'=>BRANCH_ID,'dept_id'=>$dept_id,'punchcode'=>$punchcode);
			if($password!=""){
