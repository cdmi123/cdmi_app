...
class Schoolinq extends CI_Controller {
	...
	public function add_school_inquiry($id='')
	{
		...
		if($this->input->post())
		{
			...
			$inq_year = date("Y");
			$arr = array('s_name'=>$name,'contact1'=>$contact1,'contact2'=>$contact2,'contact3'=>$contact3,'s_id'=>$school,'status'=>$status,'inq_by'=>$enq_by,'inq_year'=>$inq_year,'course'=>$course,'extra_info'=>$extra_info,'reference'=>$reference,'reference_name'=>$reference_name,'expected_date'=>$expected_join_date,'visit_date'=>$visit_date,'added_by'=>$added_by);
			if($id>0)
			{
				$up_arr = array('s_name'=>$name,'contact1'=>$contact1,'contact2'=>$contact2,'contact3'=>$contact3,'s_id'=>$school,'status'=>$status,'inq_by'=>$enq_by,'inq_year'=>$inq_year,'course'=>$course,'extra_info'=>$extra_info,'reference'=>$reference,'reference_name'=>$reference_name,'expected_date'=>$expected_join_date,'visit_date'=>$visit_date,'added_by'=>$added_by);
					$this->db->where('id',$id);
